	...
	extends EclipseLink2_0ContextModelTestCase
{
	...
	private ICompilationUnit createTestEntityOneToOneMapping() throws Exception {
		return this.createTestType(new DefaultAnnotationWriter() {
			@Override
			public Iterator<String> imports() {
				return IteratorTools.iterator(JPA.ENTITY, JPA.ONE_TO_ONE, JPA.JOIN_COLUMN, JPA.FETCH_TYPE, JPA.CASCADE_TYPE, EclipseLink.JOIN_FETCH, EclipseLink.JOIN_FETCH_TYPE, EclipseLink.PRIVATE_OWNED);
			}
			@Override
			public void appendTypeAnnotationTo(StringBuilder sb) {
				sb.append("@Entity");
	...
	public void testVirtualJoinTable() throws Exception {
		...
		ModifiableJoinColumn virtualInverseOrmJoinColumn = virtualJoinTable.getDefaultInverseJoinColumn();
		assertEquals(TYPE_NAME + "_Address", virtualInverseOrmJoinColumn.getDefaultTableName());
	...
	public void testVirtualMappingMetadataCompleteFalse() throws Exception {
		...
		ModifiableJoinColumn ormJoinColumn = 
		...
		assertEquals("MY_TABLE", ormJoinColumn.getSpecifiedTableName());
