...
public class GenericJavaAssociationOverride2_0Tests extends Generic2_0ContextModelTestCase
{
	...
	public void testRemoveSpecifiedJoinColumn() throws Exception {
		...
		SpecifiedJoinColumnRelationshipStrategy joiningStrategy = javaAssociationOverride.getRelationship().getJoinColumnStrategy();
	...
	public void testUpdateJoinColumns() throws Exception {
		...
		JavaSpecifiedJoinColumnRelationshipStrategy joiningStrategy = specifiedAssociationOverride.getRelationship().getJoinColumnStrategy();
	...
	public void testSetJoinTableName() throws Exception {
		...
		AssociationOverride javaAssociationOverride = overrideContainer.getVirtualOverrides().iterator().next();
		javaAssociationOverride = ((VirtualAssociationOverride) javaAssociationOverride).convertToSpecified();
		SpecifiedJoinTableRelationshipStrategy joiningStrategy = ((SpecifiedOverrideRelationship2_0) javaAssociationOverride.getRelationship()).getJoinTableStrategy();
		
		JavaResourceType resourceType = (JavaResourceType) getJpaProject().getJavaResourceType(PACKAGE_NAME + ".AnnotationTestTypeChild", AstNodeType.TYPE);
		AssociationOverrideAnnotation2_0 associationOverrideResource = (AssociationOverrideAnnotation2_0) resourceType.getAnnotation(0, JPA.ASSOCIATION_OVERRIDE);
