...
{
	// singleton
	private static final AnnotationDefinition INSTANCE = new EmbeddableAnnotationDefinition(JPA.JAVAX_PACKAGE);
	...
	public static AnnotationDefinition instance(String jpaPackage) {
		...
		return new EmbeddableAnnotationDefinition(jpaPackage);
	}

	private final String annotationName;

		private EmbeddableAnnotationDefinition(String jpaPackage) {
		super();
		this.annotationName = jpaPackage + EmbeddableAnnotation.ANNOTATION_NAME.substring(JPA.JAVAX_PACKAGE.length());
	}

	public Annotation buildAnnotation(JavaResourceAnnotatedElement parent, AnnotatedElement annotatedElement) {
		if (EmbeddableAnnotation.ANNOTATION_NAME.equals(this.annotationName)) {
