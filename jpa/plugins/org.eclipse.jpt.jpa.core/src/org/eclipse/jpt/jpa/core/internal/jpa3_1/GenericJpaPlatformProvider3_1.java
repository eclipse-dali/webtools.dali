...
public class GenericJpaPlatformProvider3_1 extends AbstractJpaPlatformProvider {
	...
	protected static final JpaResourceModelProvider[] RESOURCE_MODEL_PROVIDERS = new JpaResourceModelProvider[] {
			JavaResourceModelProvider.instance(), JarResourceModelProvider.instance(),
			PersistenceResourceModelProvider.instance(), OrmResourceModelProvider.instance() };

	// ********* Java managed types *********

	/**
	 * To the specified list, add java managed type definitions to use for analyzing
	 * the type given all annotations on it. The order is important, as once a
	 * managed type definition tests positive for a type, all following managed type
