...
{
	// singleton
	private static final AnnotationDefinition INSTANCE = new VersionAnnotationDefinition(JPA.JAVAX_PACKAGE);
	...
	public static AnnotationDefinition instance(String jpaPackage) {
		...
		return new VersionAnnotationDefinition(jpaPackage);
	}

	private final String annotationName;

		private VersionAnnotationDefinition(String jpaPackage) {
		super();
		this.annotationName = jpaPackage + VersionAnnotation.ANNOTATION_NAME.substring(JPA.JAVAX_PACKAGE.length());
	}

	public Annotation buildAnnotation(JavaResourceAnnotatedElement parent, AnnotatedElement annotatedElement) {
		if (VersionAnnotation.ANNOTATION_NAME.equals(this.annotationName)) {
