...
{
	// singleton
	private static final AnnotationDefinition INSTANCE = new OneToOneAnnotationDefinition(JPA.JAVAX_PACKAGE);
	...
	public static AnnotationDefinition instance(String jpaPackage) {
		...
		return new OneToOneAnnotationDefinition(jpaPackage);
	}

	private final String annotationName;

		private OneToOneAnnotationDefinition(String jpaPackage) {
		super();
		this.annotationName = jpaPackage + OneToOneAnnotation.ANNOTATION_NAME.substring(JPA.JAVAX_PACKAGE.length());
	}

	public Annotation buildAnnotation(JavaResourceAnnotatedElement parent, AnnotatedElement annotatedElement) {
		if (OneToOneAnnotation.ANNOTATION_NAME.equals(this.annotationName)) {
