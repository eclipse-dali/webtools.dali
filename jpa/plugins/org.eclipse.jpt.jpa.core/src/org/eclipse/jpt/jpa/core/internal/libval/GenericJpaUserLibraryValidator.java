/*******************************************************************************
 * Copyright (c) 2010, 2025 Oracle. All rights reserved.
	...
	implements LibraryValidator
{
	public IStatus validate(JptLibraryProviderInstallOperationConfig config) {
		...
		ArrayList<String> classNames = new ArrayList<>(3);

		if (JpaPackageHelper.isJakarta(config.getProjectFacetVersion())) {
			// JPA 3.0+: all annotations are in jakarta.persistence
			classNames.add(JPA.JAKARTA_PACKAGE + ".Entity"); //$NON-NLS-1$
		} else {
			// JPA 2.x: annotations are in javax.persistence
			classNames.add(JPA.JAVAX_PACKAGE + ".Entity"); //$NON-NLS-1$
			...
			if (config.getProjectFacetVersion().compareTo(JpaProject2_1.FACET_VERSION) >= 0) {
				classNames.add(JPA.JAVAX_PACKAGE + ".Convert"); //$NON-NLS-1$
			}
		}
