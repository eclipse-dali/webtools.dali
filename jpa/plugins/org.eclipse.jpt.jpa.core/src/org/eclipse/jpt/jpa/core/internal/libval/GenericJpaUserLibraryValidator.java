	...
	implements LibraryValidator
{
	public IStatus validate(JptLibraryProviderInstallOperationConfig config) {
		...
		if (config.getProjectFacetVersion().compareTo(JpaProject3_0.FACET_VERSION) >= 0) {
			// I believe single class check is enough .Since every class is in jakarta package.
			classNames.add("jakarta.persistence.Entity"); //$NON-NLS-1$
		} else {
