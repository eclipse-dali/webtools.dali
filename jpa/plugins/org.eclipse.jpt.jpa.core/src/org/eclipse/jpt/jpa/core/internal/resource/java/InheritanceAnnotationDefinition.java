...
{
	// singleton
	private static final AnnotationDefinition INSTANCE = new InheritanceAnnotationDefinition(JPA.JAVAX_PACKAGE);
	...
	public static AnnotationDefinition instance(String jpaPackage) {
		...
		return new InheritanceAnnotationDefinition(jpaPackage);
	}

	private final String annotationName;

		private InheritanceAnnotationDefinition(String jpaPackage) {
		super();
		this.annotationName = jpaPackage + InheritanceAnnotation.ANNOTATION_NAME.substring(JPA.JAVAX_PACKAGE.length());
	}

	public Annotation buildAnnotation(JavaResourceAnnotatedElement parent, AnnotatedElement annotatedElement) {
		if (InheritanceAnnotation.ANNOTATION_NAME.equals(this.annotationName)) {
