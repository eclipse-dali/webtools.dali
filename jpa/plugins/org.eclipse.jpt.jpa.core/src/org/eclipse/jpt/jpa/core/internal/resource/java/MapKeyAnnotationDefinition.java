...
{
	// singleton
	private static final AnnotationDefinition INSTANCE = new MapKeyAnnotationDefinition(JPA.JAVAX_PACKAGE);
	...
	public static AnnotationDefinition instance(String jpaPackage) {
		...
		return new MapKeyAnnotationDefinition(jpaPackage);
	}

	private final String annotationName;

		private MapKeyAnnotationDefinition(String jpaPackage) {
		super();
		this.annotationName = jpaPackage + MapKeyAnnotation.ANNOTATION_NAME.substring(JPA.JAVAX_PACKAGE.length());
	}

	public Annotation buildAnnotation(JavaResourceAnnotatedElement parent, AnnotatedElement annotatedElement) {
		if (MapKeyAnnotation.ANNOTATION_NAME.equals(this.annotationName)) {
