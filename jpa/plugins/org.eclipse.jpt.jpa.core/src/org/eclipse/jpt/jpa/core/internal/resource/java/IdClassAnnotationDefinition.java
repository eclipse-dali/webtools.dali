...
{
	// singleton
	private static final AnnotationDefinition INSTANCE = new IdClassAnnotationDefinition(JPA.JAVAX_PACKAGE);
	...
	public static AnnotationDefinition instance(String jpaPackage) {
		...
		return new IdClassAnnotationDefinition(jpaPackage);
	}

	private final String annotationName;

		private IdClassAnnotationDefinition(String jpaPackage) {
		super();
		this.annotationName = jpaPackage + IdClassAnnotation.ANNOTATION_NAME.substring(JPA.JAVAX_PACKAGE.length());
	}

	public Annotation buildAnnotation(JavaResourceAnnotatedElement parent, AnnotatedElement annotatedElement) {
		if (IdClassAnnotation.ANNOTATION_NAME.equals(this.annotationName)) {
