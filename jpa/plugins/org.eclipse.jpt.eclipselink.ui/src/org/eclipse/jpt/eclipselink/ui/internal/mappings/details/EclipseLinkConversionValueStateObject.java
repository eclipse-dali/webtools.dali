...
final class EclipseLinkConversionValueStateObject extends AbstractNode
{
	...
	private void addDataValueProblemsTo(List<Problem> currentProblems) {
		if (StringTools.stringIsEmpty(this.dataValue)) {
			currentProblems.add(buildProblem(EclipseLinkUiMappingsMessages.EclipseLinkConversionValueStateObject_dataValueMustBeSpecified));
		}
		else if (this.dataValues.contains(this.dataValue)) {
			currentProblems.add(buildProblem(EclipseLinkUiMappingsMessages.EclipseLinkConversionValueStateObject_dataValueAlreadyExists));
		}
	}
