	...
	extends ApplicationWindow
{
	...
	public static void main(String[] args) {
		Window window = new TreeLabelProviderUiTest();
	...
	private void buildTreePanel(Composite parent) {
		...
		this.treeViewer = new TreeViewer(panel, SWT.SINGLE | SWT.H_SCROLL | SWT.V_SCROLL | SWT.BORDER);
		this.treeViewer.getTree().setLayoutData(new GridData(GridData.FILL, GridData.FILL, true, true));
		...
		this.treeViewer.setContentProvider(contentAndLabelProvider);
		this.treeViewer.setLabelProvider(contentAndLabelProvider);
	...
	private ISelectionChangedListener buildTreeSelectionChangedListener() {
		return new ISelectionChangedListener() {
			public void selectionChanged(SelectionChangedEvent event) {
				TreeLabelProviderUiTest.this.selectedVehicleModel.setValue((Vehicle) ((IStructuredSelection) event.getSelection()).getFirstElement());
			}
		};
	}
		...
		combo.addSelectionChangedListener(
			new ISelectionChangedListener() {
				public void selectionChanged(SelectionChangedEvent event) {
					...
					if (vehicle != null) {
						vehicle.setVehicleType((VehicleType) ((StructuredSelection) event.getSelection()).getFirstElement());
					}
				}
			});
		this.selectedVehicleModel.addPropertyChangeListener(
			...
			new PropertyChangeListener() {
				public void propertyChanged(PropertyChangeEvent event) {
					...
					combo.setSelection((vehicle == null) ? EmptySelection.instance() : new StructuredSelection(vehicle.getVehicleType()));
				}
			});
	}
		...
		combo.addSelectionChangedListener(
			new ISelectionChangedListener() {
				public void selectionChanged(SelectionChangedEvent event) {
					Vehicle vehicle = TreeLabelProviderUiTest.this.getSelectedVehicle();
					if (vehicle != null) {
						vehicle.setColor((VehicleColor) ((StructuredSelection) event.getSelection()).getFirstElement());
					}
				}
			});
		this.selectedVehicleModel.addPropertyChangeListener(
			...
			new PropertyChangeListener() {
				public void propertyChanged(PropertyChangeEvent event) {
					Vehicle vehicle = TreeLabelProviderUiTest.this.getSelectedVehicle();
					combo.getCombo().setEnabled(vehicle != null);
					combo.setSelection((vehicle == null) ? EmptySelection.instance() : new StructuredSelection(vehicle.getColor()));
				}
			});
	}
	...
	private ActionContributionItem buildRefreshTreeACI() {
		Action action = new Action("Restore tree", IAction.AS_PUSH_BUTTON) {
			@Override
			public void run() {
		...
		implements ItemTreeContentProvider.Factory
	{
		public ItemStructuredContentProvider buildProvider(Object inputElement, ItemStructuredContentProvider.Manager manager) {
			return new StaticItemStructuredContentProvider(inputElement, ((Root) inputElement).getVehicles(), manager);
		}
		...
		implements ItemExtendedLabelProvider.Factory
	{
		public ItemExtendedLabelProvider buildProvider(Object item, ItemExtendedLabelProvider.Manager manager) {
			return (item instanceof Vehicle) ? this.buildProvider((Vehicle) item, manager) : NullItemExtendedLabelProvider.instance();
		}
		private ItemExtendedLabelProvider buildProvider(Vehicle vehicle, ItemExtendedLabelProvider.Manager manager) {
			return new ModelItemExtendedLabelProvider(
						vehicle,
						manager,
						buildImageDescriptorModel(vehicle),
						buildTextModel(vehicle),
						buildDescriptionModel(vehicle)
					);
		}
	}
