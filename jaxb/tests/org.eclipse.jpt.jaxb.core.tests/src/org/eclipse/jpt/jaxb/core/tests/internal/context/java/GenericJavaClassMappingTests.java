...
public class GenericJavaClassMappingTests
	...
	private void createXmlTypeWithVariousFieldsAndProperties() throws Exception {
		SourceWriter sourceWriter = new SourceWriter() {
			public void appendSourceTo(StringBuilder sb) {
				sb.append(CR);
				sb.append("import ").append(JAXB.XML_TRANSIENT).append(";").append(CR);
	...
	public void testTypeKind() throws Exception {
		...
		assertEquals(TypeKind.CLASS, classMapping.getTypeKind());
		// static - no updating/modification
	}
	...
	public void testModifyXmlTransient() throws Exception {
		...
		XmlTransientAnnotation annotation = (XmlTransientAnnotation) resourceType.getAnnotation(JAXB.XML_TRANSIENT);
		assertFalse(classMapping.isXmlTransient());
		assertNull(annotation);
		
		classMapping.setXmlTransient(true);
		annotation = (XmlTransientAnnotation) resourceType.getAnnotation(JAXB.XML_TRANSIENT);
		assertTrue(classMapping.isXmlTransient());
	...
	public void testUpdateIncludedAttributes() throws Exception {
		...
		JavaClass javaClass = (JavaClass) getContextRoot().getJavaType("test.AnnotationTestTypeChild");
		JavaClassMapping classMapping = javaClass.getMapping();
		
		assertEquals(XmlAccessType.PUBLIC_MEMBER, classMapping.getAccessType());
		
		assertEquals(2, classMapping.getIncludedAttributesSize());
		assertEquals("publicField", IterableTools.get(classMapping.getIncludedAttributes(), 0).getName());
		assertEquals("publicProperty", IterableTools.get(classMapping.getIncludedAttributes(), 1).getName());
		
		classMapping.setSpecifiedAccessType(XmlAccessType.FIELD);
		
		assertEquals(2, classMapping.getIncludedAttributesSize());
		assertEquals("publicField", IterableTools.get(classMapping.getIncludedAttributes(), 0).getName());
		assertEquals("protectedField", IterableTools.get(classMapping.getIncludedAttributes(), 1).getName());
		
		classMapping.setSpecifiedAccessType(XmlAccessType.PROPERTY);
		
		assertEquals(2, classMapping.getIncludedAttributesSize());
		assertEquals("publicProperty", IterableTools.get(classMapping.getIncludedAttributes(), 0).getName());
		assertEquals("protectedProperty", IterableTools.get(classMapping.getIncludedAttributes(), 1).getName());
		
		classMapping.setSpecifiedAccessType(XmlAccessType.NONE);
		
		assertEquals(0, classMapping.getIncludedAttributesSize());
	}
